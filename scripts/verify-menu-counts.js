...
async function verifyMenuCounts() {
    ...
    try {
        ...
        const localRestaurantsArray = parsedJson.restaurants;
        const localRestaurantCounts = new Map();
        ...
        for (const restaurant of localRestaurantsArray) {
            ...
            if (restaurant.categories && Array.isArray(restaurant.categories)) {
                ...
                else if (typeof restaurant.menu === 'object' && restaurant.menu.categories) {
                    // Case 3: 'menu' is an object containing a 'categories' array (e.g., Wen Cheng)
                    const menuObject = restaurant.menu;
        ...
        const firestoreRestaurantCounts = new Map();
        ...
        if (!discrepanciesFound) {
            ...
            if (missingInFirestore > 0)
                console.error(`  ${missingInFirestore} restaurant(s) missing in Firestore.`);
            if (missingInLocal > 0)
                console.error(`  ${missingInLocal} restaurant(s) missing in local JSON (unexpected).`);
        }
    }
    catch (error) {
